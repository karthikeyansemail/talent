...
<div class="action-bar">
    ...
    @endif
    @php $unanalyzedCount = $job->applications()->whereNull('ai_analyzed_at')->count(); @endphp
    <button type="button" id="analyzeAllBtn"
        class="action-btn"
        data-url="{{ route('applications.analyzeAll', $job) }}"
        data-csrf="{{ csrf_token() }}"
        onclick="analyzeAllCandidates(this)"
        {{ $unanalyzedCount === 0 ? 'disabled' : '' }}
        title="{{ $unanalyzedCount === 0 ? 'All candidates already analysed' : 'Queue AI analysis for all unanalysed candidates' }}">
        ...
        <span id="analyzeAllBtnText">Analyse All{{ $unanalyzedCount > 0 ? " ({$unanalyzedCount})" : '' }}</span>
    </button>
                ...
                <td>
                    <div class="table-actions">
                        ...
                        <button type="button" class="btn btn-sm btn-primary ai-analyze-btn"
                            ...
                            data-analyzed="{{ $app->ai_analyzed_at ? 'true' : 'false' }}">
