    ...
    <tr @if($match->explanation) data-expand-target="expand-match-{{ $match->id }}" @endif>
        <td>
            <div style="display:flex;align-items:center;gap:8px">
                ...
                @endif
                @php $_canViewEmp = auth()->user()->isSuperAdmin() || (auth()->user()->currentOrganization()?->canUseModule('work_signals') ?? false); @endphp
                @if($_canViewEmp)
                <a href="{{ route('employees.show', $match->employee_id) }}" class="name-link">{{ $match->employee->full_name }}</a>
                @else
                <span class="name-link">{{ $match->employee->full_name }}</span>
                @endif
            </div>
        </td>
